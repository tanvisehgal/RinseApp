...
// Conform to these delegates
class SignUpViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {

    // Subviews

    // allows users to aupload a profile picture
    private let profilePictureImageView: UIImageView = {
        ...
        // corner radius is half of height to make a circle
        imageView.layer.cornerRadius = 45
        return imageView
    }()

    private let usernameField: RinseTextField = {
    ...
    // show action sheet to take pic from camera or choose from library
    @objc func didTapImage() {
        ...
        // weak self because going to reference self and do not want to cause memory leak
        sheet.addAction(UIAlertAction(title: "Take Photo", style: .default, handler: { [weak self] _ in

            // call on main thread because UI operation
            // want to present controllers on the main thread
    ...
    @objc func didTapTerms() {
        guard let url = URL(string: "Terms") else {
            return
        }
    ...
    @objc func didTapPrivacy() {
        guard let url = URL(string: "Privacy") else {
            return
        }
        let vc = SFSafariViewController(url: url)
        present(vc, animated: true)
