...
final class ProfileViewController: UIViewController {
    ...
    private func fetchProfileInfo() {
        ...
        // Counts (3)
        group.enter()
        DatabaseManager.shared.getUserCounts(username: user.username) { [self] result in
            defer {
                group.leave()
            }
            posts = result.posts
...
extension ProfileViewController: ProfileHeaderCountViewDelegate {
    ...
    func profileHeaderCountViewDidTapFollow(_ containerView: ProfileHeaderCountView) {
        print("DEBUG: in profileheadercountviewdidtapfollow")
        DatabaseManager.shared.updateRelationship(
            state: .follow,
            for: user.username
        ) { [weak self] success in
            if !success {
                print("ProfileViewControler DidTapFollow: failed to follow")
                DispatchQueue.main.async {
                    self?.collectionView?.reloadData()
                }
            }
