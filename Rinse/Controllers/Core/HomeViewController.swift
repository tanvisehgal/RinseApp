...
final class HomeViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    private func sortData() {
        ...
        // sort array of view models
        // each view model represents a section in our feed
        viewModels = viewModels.sorted(by: { first, second in
            var date1: Date?
            var date2: Date?
            // iterate over all cell type posibilities looking for timestamp
    ...
    func updateViewModel(index: Int, isLiked: Bool) {
        ...
        if isLiked && !allPosts[index].post.likers.contains(currentUsername) {
            allPosts[index].post.likers.append(currentUsername)
        }
        if !isLiked && allPosts[index].post.likers.contains(currentUsername) {
            allPosts[index].post.likers = allPosts[index].post.likers.filter { $0 != currentUsername }
        }
...
extension HomeViewController: PostCollectionViewCellDelegate {
    func postCollectionViewCellDidLike(_ cell: PostCollectionViewCell, index: Int) {
        print("DEBUG: HomeViewCollection didlike method")
...
extension HomeViewController {
    ...
    // section layout - defines how sub items are laid out
    // items go into groups go into sections
