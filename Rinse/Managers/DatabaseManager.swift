// Interact with things in Database
...
final class DatabaseManager {
    ...
    public func getNotifications(
        ...
        print("DEBUG: in database manager getNotifications method")
        guard let username = UserDefaults.standard.string(forKey: "username") else {
            completion([])
            return
        }
        let ref = database.collection("users").document(username).collection("notifications").order(by: "date", descending: true)
        ref.getDocuments { snapshot, error in
            guard let notifications = snapshot?.documents.compactMap({
                RinseNotification(with: $0.data())
            }),
            error == nil else {
                completion([])
                return
    ...
    /// - Parameters:
    ///   - identifier: New notification ID
        ...
        case .follow:
            ...
            StorageManager.shared.profilePictureURL(for: currentUsername) { [weak self] profilePictureURL in
                ...
                let model = RinseNotification(
                    identifier: id,
                    notificationType: 3,
                    profilePictureUrl: profilePhotoUrl.absoluteString,
                    username: currentUsername,
//                    dateString: String.date(from: Date()) ?? "Now",
                    date: Date(),
                    isFollowing: isFollowingMe, // need to set
                    postId: nil,
                    postUrl: nil,
                    owner: nil
                )
                NotificationsManager.shared.create(notification: model, for: targetUsername)
    ...
    public func createComments(
        ...
        StorageManager.shared.profilePictureURL(for: currentUsername) { [weak self] profilePictureURL in
            ...
            self?.getPost(with: postID, from: owner) { post in
...
//                        dateString: String.date(from: Date()) ?? "Now",
                        ...
                        isFollowing: nil,
                        postId: postID,
                        postUrl: post.postUrlString,
                        owner: owner
                    )
                    NotificationsManager.shared.create(notification: model, for: owner)
                }
